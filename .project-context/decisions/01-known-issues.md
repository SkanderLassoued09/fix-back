...
| S4 | **Role authorization is UI-only** — ⚠️ **partiellement corrigé (2026-09-10)** | front : `src/app/shared/role-routes.ts` + `route-access.guard.ts` ; back : `RolesGuard`/`@Roles` toujours peu utilisés | **Front couvert** : un guard de route central borne chaque rôle à son propre menu, les URL des autres profils redirigent vers la page d'accueil du rôle (couvert par `qa/e2e/route-role-guard.spec.ts`, 11 tests, et par les 2 tests inversés de `02-navigation-permissions.spec.ts`). **Le back reste ouvert** : le rôle vient de `localStorage`, donc ce guard est un garde-fou d'ergonomie, pas une autorisation. Un utilisateur peut toujours appeler n'importe quelle mutation. |
...
| ~~**S12**~~ | ✅ **CORRIGÉ** (2026-09-10) — `JwtAuthGuard` applique désormais l'authentification | [jwt-auth-guard.ts](../../fix-back/src/auth/jwt-auth-guard.ts) — `handleRequest` lève `UNAUTHENTICATED` quand `err || !user`, et propage l'erreur de stratégie (auparavant ignorée) | Le contournement est fermé : un appel anonyme sur une opération gardée est refusé au lieu de s'exécuter avec `req.user === undefined`. Corrige du même coup les 500 « Cannot read properties of undefined (reading '_id') » sur `unreadNotificationCount` / `getDiStatusCounts`. Couvert par `src/auth/jwt-auth-guard.spec.ts` et par le test QA inversé `qa/tests/exploratory/02-navigation-permissions.spec.ts`. |
